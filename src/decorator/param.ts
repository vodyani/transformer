...
export function ParamCameCase(target: any, property: string, descriptor: TypedPropertyDescriptor<FunctionType>) {
...
export function ParamSnakeCase(target: any, property: string, descriptor: TypedPropertyDescriptor<FunctionType>) {
  const method = descriptor.value;
  const source = `${target.constructor.name}.${property}`;
